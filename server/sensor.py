    ...
    try:
        if os.path.exists(CALIBRATION_FILE):
            with open(CALIBRATION_FILE, "r") as f:
                ...
                print(
                    f"[DEBUG] Calibration ratio loaded from {CALIBRATION_FILE}: {ratio}"
                )
                return ratio
    except Exception as e:
...
def calibrate_start():
    ...
    try:
        ...
        print(
            f"[DEBUG] Tare complete. tare_raw={tare_raw}, offset={hx.offset}, scale={hx.scale}"
        )
        calibration_state["message"] = (
            "Tare complete. Place a known weight on the scale and press Continue."
        )
...
def calibrate_weight_read():
    if (
        not calibration_state["in_progress"]
        or calibration_state["step"] != "place_weight"
    ):
        calibration_state["message"] = (
            "Calibration step error: not ready to read weight."
        )
        calibration_state["step"] = "error"
    ...
    if raw_with_weight is not None:
        ...
        calibration_state["message"] = (
            "Enter the known weight value (grams) in the frontend."
        )
...
def calibrate_set_known_weight(value):
    if (
        not calibration_state["in_progress"]
        or calibration_state["step"] != "enter_weight"
    ):
        calibration_state["message"] = (
            "Calibration step error: not ready to set known weight."
        )
        calibration_state["step"] = "error"
    ...
    try:
        ...
        print(
            f"[DEBUG] Calibration complete. tare_raw={tare_raw}, raw_with_weight={raw_with_weight}, offset={offset}, ratio={ratio}, scale={hx.scale}"
        )
        calibration_state["step"] = "done"
        calibration_state["message"] = (
            f"Calibration complete. Ratio set to {ratio:.4f}."
        )
        calibration_state["in_progress"] = False
...
def read_mass():
    ...
    try:
        ...
        scale = getattr(hx, "scale", None)
        print(
            f"[DEBUG] read_mass: raw={raw}, offset={offset}, scale={scale}, weight={weight}"
        )
    except Exception as e:
